...
3. **Which one would you switch on for two weeks?** — the proposals that got "build it", by headline as it stood in the deck, up to three · none of them
...
skill: automation-mining {metadata.version from SKILL.md's frontmatter} · host: {Claude Code | other} · model: {if known}
...
- surfaces opened only after an ask: {yes | no — which, and what was taken as the consent}
- evidence spot-check: {N of N links resolve}
- read-only: {held | what was written, where, and on whose yes — a build on a mined surface after the verdicts goes here}
